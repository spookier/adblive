      ...
      formatter: (cellContent, row) => {
        ...
        return (
          <button
            ...
            onClick={() => handleDelete(row._id, row.nomParticipant)}> Delete
            </button >
          );},
      },
    ];
...
const handleDelete = (rowId, name) => 
{
  ...
  setData(data.filter(item => item._id !== rowId)); //on efface l'element du datagrid

  const config = {
    headers: { "Authorization": `Bearer ${token}` },
  };

  axios.delete(`/api/sejour/${rowId}`, config)
